...
Les observations $y$ suivent une distribution particulière $\mathcal{ExpFam}$ appartenant à la famille des distributions exponentielles avec des paramètres $\theta$. La distributions normale, de Poisson, Binomiale et Gamma font toutes parties de la famille des distributions exponentielles.
...
C'est ce qui permet de relier les variables explicatives à la variable réponse.
...
On a donc des observations $y$ qui suivent une distribution de Poisson avec un paramètre $\lambda$. Une fonction de lien $log$ permet de relier linéairement les variables explicatives à la valeur attendue $\lambda$ de la variable réponse.
...
Cette fonction permet de calculer la probabilité d'observer un événement $k$ fois avec une distribution de Poisson ayant un paramètre $\lambda$, autrement dit une moyenne $\lambda$.
...
Simulons une disttribution de Poisson en distribuant aléatoirement des observations dans un carré 10 x 10. Ensuite, divisons ce carré en 100 cellules.
...
legend("topright",lty=c(NA,1),legend=c("Observée","Théorique"),bty="n",cex=2,pch=c(22,21),pt.bg=c("gray","white"))
...
Les coefficients estimés sont très similaires de ce qui a été utilisé pour générer le modèle.
...
Maintenant, illustrons l'effet de la variable explicative.
...
Ceratines supposition sont les mêmes que pour un modèle linéaire standard avec une distribution, mais d'autre diffèrent.
...
Si le modèle est appropiré pour les données, on devrait voir ici une distribution uniforme des points et il ne devrait pas y avoir de patrons particulier dans la distribution des points dans le graphique de droite. En particlier, les lignes de prédictions illustrées devraient être alignées sur les trois lignes représentant les quantiles 25, 50 et 75%. Dans le graphique quantile-quantile de gauche, les points devraient être alignés sur la droite. 
...
```{r,message=FALSE,fig.height=5,results="hold"}
...
Supposons qu'on tire pile (0) ou face (1) 10 fois, qu'on calcule le nombre de face et qu'on répète cette exercice 500 fois. Quelle sera la distribution du nombre de faces obtenus?
...
C'est lorsque les données sont plus variables que ce qui est attendu par le modèle. Les distribution de Poisson et binomiales sont très restrictives et ceci est très fréquent.

En général, une surdisperion ignorée fera en sorte que l'incertitude sera sous-estimée (trop de confiance dans nos résulats).

Plusieurs causes possibles (variables manquantes, phénomènes d'aggrégation, hétérogénéité, etc.)

Solutions: Poisson -> Négative binomiale (`glm.nb`, `glmmTMB`, `brms`, etc.)

Binomiale -> Béta binomiale (`glmmTMB`, `brms`, etc.)

Obervation-level random effects (OLRE) (Harrison et al.)

### Sousdisperion

C'est lorsque les données sont moins variables que ce qui est attendu par le modèle. Ceci est beaucoup plus rare et probablement moijns dommageables (e.g. taille de couvée). 

En général, une sous-dispersion ignorée fera en sorte que l'incertitude sera surestimée (trop conservateurs dans nos résultats).

Solutions: Poisson -> `glmmTMB, family = "compois"`
 
## Zéro-inflation

Surabondance de 0 dans les observations par rapport à ce qui est attendu par le modèle. Il est possible aussi d'avoir une sous-abondance de 0 ou encore une surabondance de 1, mais c'est un peu plus rare.

Solutions: modèle *zéro-inflated* (`glmmTMB`,`pscl`,`brms`,etc.)
