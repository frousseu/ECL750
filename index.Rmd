...
* l'écart-type des moyennes obtenues si on refaisait plusieurs fois le même échantillonnage
...
plot(g,show_data=TRUE,jitter=FALSE)
...
m<-glmmTMB(y~x,family="nbinom2",data=d)
...
<br><br>
<p style='text-align: center'>Un modèle est dit hiérarchique lorsque certains de ses paramètres sont définis par une distribution de probabilité.</p>
...
set.seed(1234)
ng<-50 # nb de groupes
nobs<-50 # nb d'observations par groupe
n <- ng * nobs
id <- gl(ng, nobs)
...
plot(g, show_data = TRUE, show_ci = FALSE, colors = rep("black", 20), show_legend = FALSE)
...
g <- ggpredict(m, terms = c("x", "id [sample=20]"), type = "random")
plot(g, show_data = TRUE, show_ci = FALSE, colors = sample(colors(), 20))
...
$$\boldsymbol{y} \sim \text{Normale}(\mathbf{\boldsymbol{\beta}}\mathbf{X}, \mathbf{I}\sigma^2)$$

$$\boldsymbol{y} \sim \text{Normale}(\mathbf{\boldsymbol{\beta}}\mathbf{X}, \mathbf{\Sigma})$$


## Structure de covariance

Si on a 20 groupes avec 10 observations par groupe, la matrice de covariance $\mathbf{\Sigma}$ est diagonale par blocs avec 20 groupes identiques de taille $10 \times 10$ :
...
0 & 0 & \cdots & \mathbf{\Sigma}_{20}
\end{pmatrix}
$$

Chaque groupe a une variance totale de :

$$
\mathbf{\Sigma}_i
=
\sigma_\alpha^2 \mathbf{J}_{10}
+
\sigma_\epsilon^2 \mathbf{I}_{10}
$$

où

- $\mathbf{J}_{10}$ est une matrice $10 \times 10$ de 1  
- $\mathbf{I}_{10}$ est la matrice identité $10 \times 10$  

<br>
Pour chaque groupe on a donc:

$$
\mathbf{\Sigma}_i =
\begin{pmatrix}
\sigma_\alpha^2 + \sigma_\epsilon^2 & \sigma_\alpha^2 & \cdots & \sigma_\alpha^2 \\
\sigma_\alpha^2 & \sigma_\alpha^2 + \sigma_\epsilon^2 & \cdots & \sigma_\alpha^2 \\
\vdots & \vdots & \ddots & \vdots \\
\sigma_\alpha^2 & \sigma_\alpha^2 & \cdots & \sigma_\alpha^2 + \sigma_\epsilon^2
\end{pmatrix}
$$
...
Donc, au sein d'un même groupe :

$$
\mathrm{Var}(y_{ij}) = \sigma_\alpha^2 + \sigma_\epsilon^2
$$

$$
\mathrm{Cov}(y_{ij}, y_{ik}) = \sigma_\alpha^2
\quad (j \neq k)
$$
...
Il s’agit de la structure de covariance à symétrie composée induite par un modèle à intercept aléatoire pour mesures répétées.
...
Un seul modèle pour tous, on ignore les différences entre les groupes
<br><br><br>

_No pooling_

Un seul modèle par groupe, on estime un paramètre pour chaque groupe et ces paramètres sont indépendants
<br><br><br>

_Partial pooling_

Un seul modèle par groupe, mais les paramètres pour chaque groupe sont influencés par un modèle et les estimations sont tirées vers une valeur commune
...
set.seed(1234) #12345
ng <- 50
nobs <- sample(2:10, size = ng, replace = TRUE)

#id <- gl(ng, nobs)
id <- factor(rep(1:ng, times = nobs), levels = 1:ng)
n <- length(id)
...
# on génère l'effet aléatoire pour chaque groupe
rea <- rep(rnorm(ng, 0, 20), times = nobs)

reb <- rep(rnorm(ng, 0, 0.5), times = nobs)
...
co <- coef(m2)
co[1:2] <- c(0, 0)
names(co)[1:2] <- c("id1", "x:id1")
co <- co[order(names(co))]
mco <- matrix(co, ncol = 2)
rownames(mco) <- names(co)[1:(length(co)/2)]
mco <- mco[order(match(gsub("id", "", rownames(mco)), levels(d$id))), ]
mco <- mco + matrix(coef(m2)[1:2], ncol = 2)[rep(1, nrow(mco)), ]
#mco

plot(mco[, 1], mco[, 2], pch = 16, xlab = "Intercept", ylab = "Pente")
#text(mco[, 1], mco[, 2], labels = unname(table(id)), cex = 0.5, adj = c(0.5, -1))
points(coef(m)$id[, 1], coef(m)$id[, 2], pch = 1, col = "tomato")
abline(v = a, lty = 3); abline(h = b, lty = 3)

invisible(lapply(1:nrow(mco), function(i){
  lines(c(mco[i, 1], coef(m)$id[i, 1]), c(mco[i, 2], coef(m)$id[i, 2]), col = "red")
}))

legend("top", inset = c(0, -0.12), bty = "n", ncol = 2, legend = c("lm [ y ~ x * id ]", "lmer [ y ~ x + (x | id) ]"), pch = c(16, 1), col = c("black", "red"), xpd = TRUE)
...
plot(mco[, 1], mco[, 2], pch = 16, xlab = "Intercept", ylab = "Pente")
text(mco[, 1], mco[, 2], labels = unname(table(id)), cex = 1, adj = c(0.5, -1))
points(coef(m)$id[, 1], coef(m)$id[, 2], pch = 1, col = "tomato")
abline(v = a, lty = 3); abline(h = b, lty = 3)
...
## Structure d’erreur spatiale

Si on a:

$$\boldsymbol{y} \sim \text{Normale}(\mathbf{\boldsymbol{\beta}}\mathbf{X}, \mathbf{\Sigma})$$
<br>
on peut définir la matrice de covariance par une fonction de corrélation spatiale:
<br>

$$\Sigma_{ij} = \text{Cov}(y_i, y_j) = \sigma^2 \, \rho(d_{ij})$$

où


- $d_{ij}$ est la distance entre les localisations $y_i$ et $y_j$  
- $\sigma^2$ est la variance marginale  
- $\rho()$ est la fonction de corrélation spatiale  


## Corrélation spatiale exponentielle

$$\rho(d) = \exp\left(-\frac{d}{\phi}\right)$$
<br>
où $\phi > 0$ est l'étendue de la corrélation spatiale (_range_) et la covariance devient:
<br>
$$\Sigma_{ij} = \sigma^2 \exp\left(-\frac{d_{ij}}{\phi}\right)$$
...
rho <- exp(-d / phi)
plot(d, rho, type = "l", lwd = 2, xlab = "Distance", ylab = expression("Corrélation spatiale" ~ rho(d)))

```
L'indépendence entre les observations croit avec la distance

## Formulation matricielle

Le modèle peut être écrit sous forme matricielle :

$$\boldsymbol{y} \sim \text{Normale}(\boldsymbol{\beta}\mathbf{X}, \mathbf{\Sigma})$$
<br>
avec
<br>
$$
...
\sigma^2
\begin{pmatrix}
1 & e^{-d_{12}/\phi} & e^{-d_{13}/\phi} & \cdots \\
e^{-d_{21}/\phi} & 1 & e^{-d_{23}/\phi} & \cdots \\
e^{-d_{31}/\phi} & e^{-d_{32}/\phi} & 1 & \cdots \\
\vdots & \vdots & \vdots & \ddots
\end{pmatrix}
$$

<br><br>
On estimera ici $\sigma^2$ et le paramètre $\phi$ qui détermineront la variation entre les observations. 


## Interprétation

- Les observations proches dans l’espace sont fortement corrélées.  
- La corrélation décroît progressivement lorsque la distance augmente.  
- Lorsque $d_{ij} \to \infty$, la covariance tend vers 0.  
- Lorsque $\phi$ est grand, la dépendance spatiale s’étend sur de longues distances.
...
<br>
<p style='text-align: center'>[https://bb.olker.github.io/mixedmodels-misc/glmmFAQ.html](https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html)</p>
