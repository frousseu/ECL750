...
C'est ce qui permet de relier les variables explicatives à la variable réponse. Pour chaque distribution qui pourrait être utilisée dans de cadre d'un GLM, une fonction de lien dite canonique est utilisée par défaut, mais il existe plusieurs fonctions de liens possibles.
...
La distribution de Bernoulli est un cas particulier de la distribiution binomiale avec un tirage (e.g. un individu est vivant ou mort, un individu est mâle ou femelle, etc.)
...
# 4. Vraisemblance et statistiques bayésiennes

## Petit rappel des probabilités

Quelle est la probabilité d'obtenir 2 fois pile en tirant une pièce de monnaie 2 fois?

### Pile ou face

On doit multiplier la probabilité de chaque événement pour obtneir la probabilité combinée de ces deux événements. Si $P(pile) = 0.5$ (*i.e.* la probabilité d'otenir pile), on a:
...
<br>
Ici, $L(\theta|data)$ est la fonction de vraisemblance et l'équation précédente nous dit que la vraisemblance des paramètres conditonnelle aux paramètres $\theta$ est égale à la (densité de) probabilité des données conditionelle aux paramètres $\theta$.
...
Simulons `n = 20` des présences/absences avec une probabilité de `p = 0.5`. Ensuite, calculons la vraisemblance d'observer les données générées en fonction de plusieurs valeurs du paramètre p, qui ici est une probabilité.   
...
<br>
Le maximum de cette courbe est le maximum de vraisemblance (*maximum likelihood*), i.e. la valeur de `p` pour laquelle la probabilité d'observer les données est la plus grande. Dans ce cas précis, cette valeur est la proportion de 1 dans les données.


#### Avec un glm

Voici ce qu'on obtient avec un glm si on tente d'estimer cette probabilité. En principe, l'intercept représente cette probabilité, mais elle est sur l'échelle logit. Il faut donc retransformer le tout sous l'échelle d'une probabilité avec une fonction logit inverse.
...
On peut également estimer un intervalle de confiance autour de cette probabilité avec la fonction `confint.`
...
<br><br>
$P(A|B)$ veut dire la probabilité de $A$ sachant $B$. C'est une probabilité conditionelle.
...
<br>
Dans ce cas, la probabilité d'avoir réellement la Covid-19 si le test est positif n'est que d'environ 3.8 %


### Appliqué aux statistiques
...
$$P(\theta|data) = \frac{P(data|\theta)P(\theta)}{P(data)}$$
<br><br>
La distribution postérieure $P(\theta|data)$ est ce que l'on cherche à estimer. Il s'agit d'une distribiution de probabilité qui décrit les probabilités d'observer les différentes valeurs possibles du paramètre.
...
$$P(\theta|data) = \frac{P(data|\theta)P(\theta)}{P(data)}$$
<br><br>
Les distributions *a priori* $P(data)$ représentent nos croyances sur les valeurs des paramètres avant de faire les analyses. On les représente à l'aide de distribution de probabilité. Ces croyances représentent ce à quoi on s'attend en termes de valeurs en se basant sur nos connaissances théoriques avant de faire notre expérience.
...
<br><br>
Cette quantité $P(data$ est beaucoup plus difficile à comprendre intuitivement. Elle représente en quelque sorte la probabilité des données pour l'ensemble des valeurs de paramètres possibles (d'où l'intégrale). C'est la distribution marginale des données ou la vraisemblance marginale. Elle permet également de faire en sorte que la distribution postérieure soit une distribiution de probabilité (i.e. aire sous la courbe = 1). 
<br><br>
Il s'agit d'une constante qui est généralewment très difficile à calculer. Des méthodes comme le MCMC permettre d'éviter de la calculer et de tout de même obtenir la distirbution postérieure.  
...
- Facilite l'estimation de quantité dérivées (e.g. prédictions ou combinaisons de prédictions)

- Le fait d'avoir à spécifier des priors nous force à comprendre ce que nos paramètres représentent
 
- Façon de penser plus naturelle, philosophiquement plus satisfaisant? 
...
- Nécessité de choisir des *priors*
...
par(mfrow=c(1,3))
curve(dnorm(x,0,50),from=-200,to=200,main="Beta normal(0,50)",ylab="Density")
curve(dnorm(x,0,5),from=-20,to=20,main="Intercept normal(0,5)",ylab="Density")
curve(dgamma(x,2,0.1),from=0,to=100,main="Écart-type gamma(2,0.1)",ylab="Density")

```

<br>
Par exemple, pour la variance/écart-type du modèle, il faut idéalement un *prior* avec des valeurs strictement positive, car les valeurs négatives ne font pas de sens pour la variance. Une des possibilité est d'utiliser la distribution gamma.
...
m <- brm(y ~ x,
       ...
       file = "C:/Users/User/Downloads/brm_model"
     )
...
Avec les modèles bayésiens, il faut toujours vérifier la convergence des chaînes.
...
- Régularisant (regularizing): permet de circonscrire les valeurs obtenues et de pénaliser les valeurs élevées.
...
### Quels *priors* utilisés?

Il faut aussi sélectionner une distribution approriée pour représenter nos *priors* et les distributiuons utilisées doivent êytre appropriées pour un paramètre donnée. Le groupe qui travaille sur la plateforme [STAN]() a plusieurs recommandations de *priors* à utiliser dépendemment du contexte (*priors* pour des pentes, *priors* pour des variances, *priors* pour des modèles hiérarchiques, etc.)

Voir [Prior Choice Recommendations](https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations).

### Incertitude



### Modèles compexes et hiérarchiques
