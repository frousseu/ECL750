...
Les méthodes d'analyses statistiques utilisées et la façon de rapporter les résultats devraient être influencés par le but de l'analyse.
...
Ici, on a généralement des hypothèses a *priori* que l'on veut tester avec des données. L'inférence est la priorité. 
...
En inlcuant trop de variable dans un modèle, on augmente l'incertitude dans nos estimations.
...
Un modèle surparamétrisé est un modèle qui "colle" trop bien aux données. Soit que le modèle a trop de paramètres, soit le modèle est trop flexible et s'adapte trop bien au jeu de données en question. 

Le danger est qu'en apprenant un jeu de données trop bien, le modèle risque de capturer les caractéristiques d'un jeu de données en particulier ce qui fera en sorte que le modèle sera difficilement généralisable à d'autres situations (e.g. faire de bonnes prédictions avec d'autres données).


## Parsimonie

Le principe de parsimonie guide la majeure partie de la science. ce principe veut que les explications simples sont généralement préférables aux explications complexes (ou compliquées ?).

En statistiques, un modèle parsimonieux est un modèle qui explique beacoup avec peu de paramètres.
...
```{r}
#############################################
#Rappel du R^2 (coefficient de determination).
...
Calculons un R2 à partir de cette régression. Il y a plusieurs façons de calculer le R2.
...
```{r}
#Selectionner un modèle sur la base du R^2. Favorise le surparametrage des modèles et la découverte d'effets fortuits.
#Exemple1
...
Que se passe-t-il si on ajoute presque autant de variable explicative que d'observations?
...
On voit que le R carré augmente à chaque ajout de variable.
...
Cette version est le $AIC$ corrigé pour les petites tailles d'échantillon.
...
Le principe derrière la validation croisée est d'entraîner un modèle sur une partie des données et de tenter de prédire les valeurs des données qui n'ont pas servi à entrainer le modèle.

Ceci permet de quantifier l'erreur de prédiction avec des mesures comme le *root mean square error* (*RMSE*). Ici, on peut par exemple chercher le modèle minimisant cette erreur.
...
Ici, on entraîne le modèle sur toutes les observations sauf une et on tente de prédire cette obsrvation mise de côté. On répète ce processus pour toutes les observations. 

<br>

*K-fold Cross-Validation*

Ici, on divise le jeu de données en k parties qu'on laisse de coté et on entraîne le modèle suer le reste des données. On répète le processus pour chacune des k parties.
...
Avec la *ridge regression*, on ajoute une pénalité constituée de la somme des paramètres au carré et d'un paramèytre $\lambda$ servant à faire l'ajustement. Le paramètre $\lambda$ est ajusté par validation croisé afin d'identifier la valeur optimale permettant le meilleur pouvoir prédictif.
...
Le LASSO (*Least Absolute Shrinkage and Selection Operator*) ressemble beaucoup à la *ridge regression*, mais cette fois la pénalité est construire avec la somme en valeur absolue des paramètres. 
...
La méthode *Elasticnet* est une sorte de compromis entre le LASSO et la *ridge regression*. Toutes ces méthodes sont accessibles via le package [glmnet]().
...
Le principal désavantage de ces méthodes est qu'il n'est pas clair pour l'instant comment devrait être estimé l'incertitude autour des paramètres ce qui en fait des outils moins utiles pour l'inférence.
